...
export class OrderViewPageComponent {
  ...
  cancelBooking(bookingId: string, carId: number): void {
    ...
    if (cancellationCharges > 0) {
      const refundAmount =
        this.bookingdetail.formDetails.rentalRate - cancellationCharges;

      const userStatus = `Cancelled`;
      const status = `User cancelled this booking. Refund amount: Rs.${refundAmount.toFixed(
        2
      )}, Cancellation charges: Rs.${cancellationCharges.toFixed(2)}`;
      if (
        confirm(
          `Cancellation charges of Rs.${cancellationCharges.toFixed(
            2
          )} is need to pay for cancellation. Are you sure you want to cancel?`
        )
      ) {
        this.authservice
          .updateBookingStatus(bookingId, status, userStatus)
          .subscribe(() => {
            this.authservice
              .updateCarDetailsEnableStatus(carId)
              .subscribe(() => {
                this.fetchBookings();
