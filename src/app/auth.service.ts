...
constructor(private router: Router,private http:HttpClient) {
  ...
  this.loggedInUser = loggedInUserString ? JSON.parse(loggedInUserString) : null;
}
...
getBookingsForUser(): Observable<any[]> {
  ...
  if (loggedInUser) {
    return interval(1000).pipe(
      startWith(0),
      switchMap(() =>
        this.http.get<any[]>(`${this.apiUrl}/bookings`).pipe(
          map(bookings => bookings.filter(booking => booking.user.id === loggedInUser.id))
        )
      )
    );
  }
